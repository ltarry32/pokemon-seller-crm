...
import { computeSalesKPIs } from '@/data/mockSoldTransactions';

export default function SoldPage() {
  const { soldTransactions, deleteSoldTransaction, soldLogFilters, setSoldLogFilters } = useAppStore();
  const [showAddSale, setShowAddSale] = useState(false);

  const filtered = useMemo(() => {
    let result = [...soldTransactions];
    ...
    if (soldLogFilters.sort === 'highest_profit') result.sort((a, b) => b.net_profit - a.net_profit);
    if (soldLogFilters.sort === 'highest_sale') result.sort((a, b) => b.sold_price - a.sold_price);
    return result;
  }, [soldTransactions, soldLogFilters]);

  const kpis = computeSalesKPIs(filtered);

  return (
    <div className="page-container space-y-4 animate-fade-in">
      <PageHeader
        title="Sold Log"
        subtitle={`${filtered.length} completed sales`}
        actions={
      ...
      <div className="grid grid-cols-2 gap-3">
        ...
        <StatCard
          ...
          sub={`${formatPercent(kpis.profitMargin)} margin`}
          trend="up"
        />
      </div>
      ...
      <div className="flex gap-2">
        <Input
          ...
          rightIcon={soldLogFilters.search ? (
            <button onClick={() => setSoldLogFilters({ search: '' })} className="pointer-events-auto">
        ...
        />
        <select
          value={soldLogFilters.platform}
          onChange={e => setSoldLogFilters({ platform: e.target.value as typeof soldLogFilters.platform })}
        ...
        <select
          value={soldLogFilters.sort}
          onChange={e => setSoldLogFilters({ sort: e.target.value as typeof soldLogFilters.sort })}
          className="h-10 px-3 rounded-xl bg-surface-2 border border-zinc-700 text-sm text-zinc-300 focus:outline-none focus:border-brand-500"
        >
        ...
        <div className="space-y-2">
          {filtered.map(tx => (
            <SoldCard
              key={tx.id}
              transaction={tx}
              onDelete={() => deleteSoldTransaction(tx.id)}
            />
          ))}
        </div>
      )}
      ...
      <Modal
        ...
        title="Log a Sale"
        subtitle="Record a completed transaction"
        size="lg"
      >
